...
use crate::component::{Profile, Parse,  Entity, Request, Response, Task, UserAgent};
    ...
    loop {
        ...
        match term.load(Ordering::Relaxed) {
            SIGINT => {
                ...
                //pline.process_item(cbase_result);
                //pline.process_yielderr(cbase_yield_err);
            }
            ...
            0 => {
                ...
                let john = task::spawn(async move {
                    Response::exec_all(futs, tbase_res).await;
                });
